...
const index = () => {
  ...
  const addFakeProperties = (properties) => {
    ...
    const filteredFakeProperties = fakeProperties.filter((property) => {
      ...
      if (
        (!rentalYield ||
          (property.Financial.Rental_yield >=
            parseFloat(rentalYield.split("-")[0]) &&
            property.Financial.Rental_yield <=
              parseFloat(rentalYield.split("-")[1]))) &&
    ...
    <div className={css.container}>
      ...
      <div className={css.filtersContainer}>
        <div className={css.filters}>
          ...
          <select
            value={selectedFilters.rentalYield}
            onChange={(e) => handleFilterChange("rentalYield", e.target.value)}
          >
            <option hidden value="">
              Rental Yield{" "}
            </option>
          ...
          <select
            value={selectedFilters.financeType}
          ...
          >
            ...
            <option value="Capital Rep. + Passive Income">
              Capital Rep. + Passive Income
            </option>
          </select>

          <div className={css.filterbuttons}>
            <button>all</button> |<button>New</button>|<button>Upcoming</button>
          </div>

          {/* Botón para remover los filtros */}
          <button onClick={handleRemoveFilters} className={css.clean}>
            <FiRefreshCcw />
          </button>
        </div>
      </div>
          ...
          fakeProperties.length || filteredProperties.length ? (
            addFakeProperties(filteredProperties).length ? (
              ...
              <div className={css.error}>
                <p>
                  Sorry, it seems that there are no properties available with
                  these requirements. Please try again with different
                  requirements.
                </p>
