...
async function getNextClaimableTime(propertyContractAddress: string, tokenId: number[]) {
  const propertyContract: any = await getPropertyContract(propertyContractAddress);

  const nextClaimTime = await propertyContract.getNextEligibleClaimTime(tokenId[0]);
