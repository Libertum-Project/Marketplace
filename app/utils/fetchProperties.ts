...
export async function getPropertyDetails(id: number) {
  const data = await fetch(`https://libertum--marketplace.azurewebsites.net/properties/${id}`, requestOptions);
...
export const filterProperties = (
  ...
  return properties.filter((property: any) => {
    ...
    const annualYieldMatch =
      annualYieldFilter === '' || parseFloat(property.annual_yield) >= parseFloat(annualYieldFilter);
