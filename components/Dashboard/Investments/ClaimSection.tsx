...
const ClaimSection = ({ propertyAddress: propertyContractAddress, durationInMonths }: any) => {
  ...
  useEffect(() => {
    async function fetchTokenIds() {
      if (!isUserBalanceLoading && userBalance) {
        ...
        let tokenIds: any = await getTokensIds(tokenQuantity, userWalletAddress, propertyContractAddress);
        ...
        const claimableAmountArray: number[] = [];
        const monthsToClaimArray: number[] = [];

        for (tokenIds of groupTokenIds) {
          const groupClaimableAmount: number = await getClaimableAmount(propertyContractAddress, tokenIds);
          claimableAmountArray.push(groupClaimableAmount);

          const monthsToClaim: number = await getMonthsToClaim(propertyContractAddress, tokenIds);
          monthsToClaimArray.push(monthsToClaim);
        }

        const totalClaimableAmount = claimableAmountArray.reduce((total, amount) => total + amount, 0);
        const totalMonthsToClaim = monthsToClaimArray.reduce((total, amount) => total + amount, 0);

        setClaimableAmount(totalClaimableAmount);
        setMonthsToClaim(totalMonthsToClaim);

        const nextClaimTimes: Date[] = [];
        for (const tokenIds of groupTokenIds) {
        ...
        const closestTime = nextClaimTimes.reduce((prev, curr) =>
          Math.abs(curr.getTime() - new Date().getTime()) < Math.abs(prev.getTime() - new Date().getTime())
            ? curr
            : prev,
        );
      ...
      {groups &&
        groups.map((group: any, index: number) => (
          <div key={index} className="border p-4 my-2">
            ...
            <p>Remaining moths to claim: {durationInMonths - group.numberOfClaims}</p>
            <p>Claimable amount: {group.claimableAmount}</p>
            <p>Next claimable time: {group.nextClaimTime.toLocaleDateString()}</p>
          </div>
        ))}
    </section>
  );
