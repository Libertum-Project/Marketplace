...
const ArtGridCard: React.FC<ArtCardProps> = ({ artPiece }) => {
    ...
    <>
      ...
      <CardFooter className="w-full px-4 h-fit">
        {!isExpanded ? (
          ...
          <Accordion type="single" collapsible className="w-full" value={`item-${artPiece.id}`}>
            <AccordionItem value={`item-${artPiece.id}`}>
              <AccordionContent>
